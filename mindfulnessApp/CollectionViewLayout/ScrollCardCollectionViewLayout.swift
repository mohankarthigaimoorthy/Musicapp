...
class ScrollCardCollectionViewLayout: UICollectionViewFlowLayout {
   ...
   override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
       ...
       for itemAttributes in attributes! {
           let itemAttributesCopy = itemAttributes.copy() as!
           UICollectionViewLayoutAttributes; changeLayoutAttributes(attributes: itemAttributesCopy)
   ...
   func changeLayoutAttributes(attributes:UICollectionViewLayoutAttributes) {
       ...
       let alpha = ratio * (1 - self.standardItemAlpha) +
       self.standardItemAlpha
       let scale = ratio * (1 - self.standardItemScale) +
       self.standardItemScale
